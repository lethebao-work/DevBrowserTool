...
- Mọi thuật ngữ viết hoa có định nghĩa chính thức tại Mục 14 (Bảng thuật ngữ). Nếu 1 từ viết hoa xuất hiện mà không có trong Mục 14, đây LÀ LỖI của tài liệu.
...
- Mỗi node có số lượng `Variant` tối đa (ví dụ 5). Biến thể mới vượt ngưỡng → thay thế biến thể yếu nhất (theo confidence/tần suất quan sát gần nhất).
...
1. **JS-query**: `querySelector`/tương đương qua `Runtime.evaluate`. Mặc định cho mọi trường hợp thông thường.
2. **CDP DOM-domain query**: dùng domain `DOM` của CDP (không qua JS), BẮT BUỘC dùng khi nghi ngờ có Shadow DOM đóng (closed) — vì tier 1 không xuyên qua được ranh giới này. Việc truy cập vào closed shadow root là hành động `elevated` (Mục 6.2).
3. **Fuzzy local-heal**: so khớp gần đúng (text/thuộc tính tương tự, ngưỡng tin cậy) dựa trên DOM hiện tại, không cần mạng, không cần LLM, không ghi gì mới vào Map.
4. **Computer-vision/coordinate (`click_coordinate`)**: click theo toạ độ tuyệt đối/tương đối, dùng khi trang render bằng canvas/WebGL (không có DOM để truy vấn) hoặc khi mọi tier DOM-based đều thất bại. Toạ độ tính từ vị trí đã biết của đối tượng trong game/app state (không phải đoán mù).
...
- **`elevated`**: hành động rủi ro cao nhưng CÓ THỂ tự động hoá được sau khi đã xin xác nhận người dùng ít nhất 1 lần (ví dụ truy cập closed shadow root, `patch_runtime`). Gồm: mọi truy cập closed shadow root, mọi `patch_runtime`.
...
| Giai đoạn | Phạm vi |
|---|---|
| **0 — Nền tảng** | Action primitive cơ bản (`navigate`, `click`, `fill`, `call_api`, `extract`) chỉ dùng tier JS-query; chỉ có Resource Graph (chưa có State Graph); khám phá CHỈ qua demo người dùng (chưa có agent-first tự động); Map lưu local, ghi atomic, có `schema_version`+`content_revision` ngay từ đầu; CHỈ đóng gói dạng Chrome Extension |
